...
const login = (req, res) => {
	...
	const thisSID = username + Date()
	//Success, set cookie and respond
	//TODO: Unsure if a good idea (probs not from sec standpoint, but sid is just hash
	//Store sid on redis
	redis.hmset(thisSID, userObj)

	redis.hgetall(thisSID, function(err, userObj){
		console.log(thisSID + ' mapped to: ' + userObj)
	})

	//Store sid as cookie
	res.cookie('cookieName', thisSID, {maxAge: 3600*1000})
